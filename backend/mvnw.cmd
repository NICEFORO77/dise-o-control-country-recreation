...
try {
  ...
  try {
    New-Item -Path "$MAVEN_HOME" -ItemType Directory -Force | Out-Null
    Copy-Item -Path "$TMP_DOWNLOAD_DIR/$MAVEN_HOME_NAME\*" -Destination "$MAVEN_HOME" -Recurse -Force | Out-Null
  } catch {
    if (! (Test-Path -Path "$MAVEN_HOME" -PathType Container)) {
